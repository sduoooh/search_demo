...
import { replaceNoscriptImagesZH } from '../utils.js'
...
class ZhihuSearch {
    ...
    get_xzse96(path) {
            ...
            return (tt, te, tr) => {
                var tu = function (tt, te) {
                    ...
                    , tO = function (tt, te, tr) {
                        return te ? tr ? tS(te, tt) : tT(te, tt) : tr ? tC(tt) : tE(tt)
                    };
        ...
        function ed(tt, te, tr, ti) {
            var ta = xzse93 // "101_3_3.0"
                , tu = dc0 // cookies["d_c0"]
                , tc = null // cookies["zst_81"]
                , tf = t3(tt)
                , td = t6(te)
        ...
        var t6 = function (tt) {
            return null == tt ? "" : "string" == typeof tt ? tt : "undefined" != typeof URLSearchParams && (0,
                tc._)(tt, URLSearchParams) ? tt.toString() : tm()(tt) ? JSON.stringify(tt) : t4(tt) ? String(tt) : ""
        }
        var t7 = function (tt) {
        ...
        var tm = () => function (tt) {
            ...
            return "function" == typeof tr && ti._(tr, tr) && th.call(tr) == tA
        }
        var tT = ed(
            path,
            undefined,
        ...
        try {
            const res = await fetch(backendUrl, {
                ...
                body: JSON.stringify({
                    ...
                    headers: {
                        "x-zse-96": xzse96,
                        "Referer": `https://www.zhihu.com/search?type=content&q=${query}`,
                        "User-Agent": UA,
                        "x-zse-93": xzse93,
                        "cookie": cookies,
                    }
            ...
            return JSON.parse(data.content).data
                .filter(i => i.type === 'search_result' && (i.object.type === 'answer' || i.object.type === 'article'))
                .map(i => {
                    return {
                        content: {
                            text: replaceNoscriptImagesZH(i.object.content),
                            author: i.object.author.name
                        },
                        highlight: {
                            title: i.highlight.title,
                            description: i.highlight.description,
                            thumbnails: i.object.thumbnail_info.thumbnails.map(i => i.url)
                        }
                    }
                })
        }
